...
public class SetFloatToArrayInstruction implements VMInstruction {
    VMRegistry op1;
    VMInstructionParam op2;
    VMInstructionParam op3;

    public SetFloatToArrayInstruction(JSONArray params, List<VMRegistry> registries) throws JSONException {
        if (params.length() != 3) {
    ...
    public void execute() throws VMException {
        ...
        if (firstData instanceof java.util.Map<?, ?>) {
            ((Map<Integer, Float>) firstData).put(op2.getIntegerValue(), op3.getFloatValue());
            return;
        }
        throw new VMException("Cannot set float to array!");
    }
