...
public class InitBooleanInstruction implements VMInstruction {
    VMRegistry registry;
    VMInstructionParam value;

    public InitBooleanInstruction(JSONArray params, List<VMRegistry> registries) throws JSONException {
        if (params.length() != 2) {
    ...
    public void execute() throws VMException {
        registry.setData(value.getBooleanValue());
    }
