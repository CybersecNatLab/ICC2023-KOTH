...
public class AddInstruction implements VMInstruction {
    VMRegistry op1;
    VMInstructionParam op2;
    VMRegistry op3;

    public AddInstruction(JSONArray params, List<VMRegistry> registries) throws JSONException {
        if (params.length() != 3) {
    ...
    public void execute() throws VMException {
        ...
        if (firstData instanceof String) {
            op3.setData(op1.getString() + op2.getStringValue());
            return;
        }
        throw new VMException("Cannot add types!");
    }
