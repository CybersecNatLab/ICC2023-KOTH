...
public class NegInstruction implements VMInstruction {
    VMRegistry op1;
    VMRegistry op2;

    public NegInstruction(JSONArray params, List<VMRegistry> registries) throws JSONException {
        if (params.length() != 2) {
    ...
    public void execute() throws VMException {
        ...
        if (firstData instanceof Integer) {
            op2.setData(op1.getInteger() * -1);
            return;
        }
        if (firstData instanceof Float) {
            op2.setData(op1.getFloat() * -1);
            return;
        }
        if (firstData instanceof Boolean) {
            op2.setData(!op1.getBoolean());
            return;
        }
        throw new VMException("Cannot neg types!");
    }
