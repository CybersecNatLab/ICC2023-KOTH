...
public class SetIntegerToArrayInstruction implements VMInstruction {
    VMRegistry op1;
    VMInstructionParam op2;
    VMInstructionParam op3;

    public SetIntegerToArrayInstruction(JSONArray params, List<VMRegistry> registries) throws JSONException {
        if (params.length() != 3) {
    ...
    public void execute() throws VMException {
        ...
        if (firstData instanceof java.util.Map<?, ?>) {
            ((Map<Integer, Integer>) firstData).put(op2.getIntegerValue(), op3.getIntegerValue());
            return;
        }
        throw new VMException("Cannot set integer to array!");
    }
