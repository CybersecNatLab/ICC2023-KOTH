...
public class InitIntegerInstruction implements VMInstruction {
    VMInstructionParam value;
    VMRegistry registry;

    public InitIntegerInstruction(JSONArray params, List<VMRegistry> registries) throws JSONException {
        if (params.length() != 2) {
    ...
    public void execute() throws VMException {
        registry.setData(value.getIntegerValue());
    }
