...
public class RoundInstruction implements VMInstruction {
    VMRegistry op1;
    VMRegistry op2;

    public RoundInstruction(JSONArray params, List<VMRegistry> registries) throws JSONException {
        if (params.length() != 2) {
    ...
    public void execute() throws VMException {
        ...
        if (firstData instanceof Float) {
            op2.setData(Integer.valueOf(Math.round(op1.getFloat())));
            return;
        }
