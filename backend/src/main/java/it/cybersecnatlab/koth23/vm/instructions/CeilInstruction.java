...
public class CeilInstruction implements VMInstruction {
    VMRegistry op1;
    VMRegistry op2;

    public CeilInstruction(JSONArray params, List<VMRegistry> registries) throws JSONException {
        if (params.length() != 2) {
    ...
    public void execute() throws VMException {
        ...
        if (firstData instanceof Float) {
            op2.setData(Integer.valueOf((int) Math.ceil(op1.getFloat())));
            return;
        }
